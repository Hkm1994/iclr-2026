"""
strong_mlp_knn variant: same neighbor attention + per-τ neighbor features, plus
per-point distance-to-surface geometry and one residual message-passing block.
"""
...
def _distance_to_surface_features(
    ...
    """
    Per-point min distance to any airfoil surface point and log1p(distance).
    ``pos`` (N, 3); returns (N, 2). Empty surface idcs -> zeros.
    """
    n = pos.shape[0]
    device, dtype = pos.device, pos.dtype
    out = torch.zeros(n, 2, device=device, dtype=dtype)

    # strong_mlp_knn_mp is used with batch loop per sample; this is single-cloud (N,3)
    idcs = idcs_airfoil[0] if idcs_airfoil else torch.tensor([], device=device, dtype=torch.long)
    if idcs.numel() == 0:
        return out
    surf = pos[idcs.long()]  # M, 3
...
def _knn_neighbor_tensors(
    ...
    *,
    ...
    return idx, nbr6, per_tau_flat


class StrongMLPKnnMP(GramForecastModel):
    """
    kNN attention + per-τ neighbor context, distance-to-surface (2), then embed,
    one residual neighbor message block, merged trunk, per-timestep heads.
    """

    geom_channels = 2
    # 56 (same as strong_mlp_knn raw concat) + 2 geom = 58 before embed
    raw_feature_dim = 58
    ...
    def __init__(self, config: Optional[dict[str, Any]] = None):
        ...
        if len(trunk) != 4:
            raise ValueError(
                "strong_mlp_knn_mp expects num_channels as 4-tuple "
        ...
        self.norm_mp = LayerNorm(d_mp)
        self.merge = Linear(raw_d + d_mp, d_merge)
        ...
        if cfg.get("skip_weights"):
            ...
            weight_path = os.path.join("models", "strong_mlp_knn_mp", "state_dict.pt")
        else:
    ...
    def _default_trunk(d_mp: int) -> tuple[int, int, int, int]:
        d_merge = StrongMLPKnnMP.raw_feature_dim + d_mp
    ...
    def _mp_step(
        ...
        """x_raw (N, raw_d), idx (N,k), delta (N,k,3) -> updated hidden (N, d_mp)."""
        h = self.embed(x_raw)
        h_i = h.unsqueeze(1).expand(-1, idx.shape[1], -1)
        h_j = h[idx]
        edge = torch.cat([h_i, h_j, delta], dim=-1)
        msg = self.edge_mlp(edge).mean(dim=1)
        upd = self.node_mlp(torch.cat([h, msg], dim=-1))
        return self.norm_mp(h + upd)
    ...
    def forward(
        ...
        if num_t_in != self.num_output_timesteps:
            raise ValueError(
                f"strong_mlp_knn_mp expects num_output_timesteps={self.num_output_timesteps} "
        ...
        for b in range(batch_size):
            ...
            idx, nbr6, per_tau = _knn_neighbor_tensors(
                pos[b],
                velocity_in[b],
                vel_mean[b],
                k_eff,
            ...
            nbr_attn = self._neighbor_attention(
            ...
            delta = nbr6[:, :, 3:6]
            h_mp = self._mp_step(x_raw, idx, delta)
            x = self.merge(torch.cat([x_raw, h_mp], dim=-1)).unsqueeze(0)
